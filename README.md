...
* *README last updated*: 2026/08/04
...
2. Clone the repository and navigate to the repo root.
3. Run `uv sync` to create a virtual environment and install all dependencies. *Note: if you are working within a Microsoft OneDrive-linked system, you may need to run a different version to copy instead of hard-link: `uv sync --link-mode=copy`*
4. Open the repo root folder in your IDE (e.g., **File → Open Folder** in VS Code) to ensure the environment is detected automatically (alternatively use the terminal to change directory if you are running via terminal).
...
| `env-template.json` | env file with most parameters. Should be populated with institution-specific information and renamed as `.env`. For some fields, the UT Austin-specific information is included in the template as a model of how information should be formatted. |
...
| `datacite-ror-query.py` | Contains a trimmed version of the main workflow and uses the ROR identifier (specified in `.env`) to search for affiliated datasets. The only purpose for this script is to quantify the degree to which a ROR-based query will result in an incomplete retrieval due to lack of widespread adoption of ROR. |

### accessory-scripts/deprecated
These scripts are no longer actively developed and may not be compatible with the latest version of the set-up.
...
`dataset-records-retrieval.py` now contains the Crossref retrieval and cleaning process that was previously externalized as an accessory script (`crossref-query.py`). 
